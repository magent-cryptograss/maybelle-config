...
/**
 * Check if request is from localhost (trusted internal service)
 */
 ...
 * Bypasses auth for localhost connections (trusted internal services like sync-releases)
 */
export function requireWalletAuth(req, res, next) {
    // Trust localhost connections (e.g., sync-releases.js running on same box)
    if (isLocalhost(req)) {
        req.verifiedAddress = 'localhost';
        return next();
    }
