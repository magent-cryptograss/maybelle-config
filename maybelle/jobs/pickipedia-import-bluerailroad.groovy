            ...
            script('''
                pipeline {
                    ...
                    stages {
                        ...
                        stage('Enrich thumbnails') {
                            steps {
                                sh 'set +x && /opt/blue-railroad-import/bin/python -m blue_railroad_import.cli enrich-thumbnails --wiki-url "$WIKI_URL" --username "$BLUERAILROAD_BOT_USERNAME" --password "$BLUERAILROAD_BOT_PASSWORD" -v'
                            }
                        }
